...
\begin{document}
	...
	\begin{enumerate}[(I)]
		...
		\begin{itemize}
			...
			$$
			in which $\d_n$ acts by taking cells to their boundaries, where each cell is counted \textit{with degree}. $\d_n$ is a linear map $\Z^{c_n}\to \Z^{c_{n-1}}$, so we can see $\d_n$ as a $c_n\times c_{n-1}$ matrix.
	...
	\begin{itemize}
		...
		\item \textit{Cellular Approximation}: $\pi_j(X)=0$ if $X$ has no $j$-cells.
	...
	\begin{enumerate}[(I)]
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item If $X$ has a contractible covering space then $\pi_n(X)=0$ for $n\geq 2$.
				\item If $X$ has a universal cover $\tilde{X}$ (so that $\pi_1(\tilde{X})=0$) then $\pi_2(X)=\pi_2(\tilde{X})=H_2(\tilde{X})$.
			\end{itemize}
		\end{itemize}
		\item \textbf{Quotients / Excision}:
		\begin{itemize}
			\item Express $X$ as a quotient $Y/A$, where $Y$ is a CW complex and $A$ a subcomplex.
			\item If $(Y,A)$ is $r$-connected and $A$ is $s$-connected, then the map
			$$
			\pi_j(Y,A)\to \pi_j(X)
			$$
			induced by the quotient map $Y\to X$ is an isomorphism for $j\leq r+s$ and a surjection for $j=r+s+1$.
			\item The relative homology groups $\pi_i(Y,A)$ are potentially easier to compute if you have information about $\pi_*(A)$ and $\pi_*(Y)$, as we have the long exact sequence
			$$
			...
			\item \textit{Special Cases}:
			\begin{itemize}
				\item If $Y$ is $r$-connected and $A$ is $s$-connected where $s\geq r$, then it follows from the long exact sequence that $(Y,A)$ is $r$-connected so the theorem holds. If $r>s$, in particular if $Y$ is contractible, then $(Y,A)$ is $s$-connected.
			\end{itemize}
...
%		\item For any sequence of groups $G_1,G_2,\dots$ where $G_n$ is Abelian for $n\geq 2$, there is a  
	...
	\underline{How to Construct Homotopy Equivalences}:
	\begin{itemize}
		...
		\item It also suffices to show that $f_*:H_*(X)\to H_*(Y)$ is an isomorphism in all dimensions.
	\end{itemize}
	\newpage
