...
\begin{document}
	...
	\begin{itemize}
		...
		\item If $X$ is a manifold with boundary, then $\chi(\partial X)=2\chi(X)$. 
		\end{itemize}
	\end{itemize}
	...
	\underline{How to Calculate $H^*(X;G)$}:
	\begin{enumerate}[(I)]
		\item \textbf{Universal Coefficient Theorem}:
		\begin{itemize}
...
%	\end{itemize}
	...
	\begin{proof}
		...
		If no root exists then $p$ is a continuous map from $\C$ to $\C\setminus \{0\}$, which is in general possible (constant maps, e.g.). Then one can compose this with a projection onto $S^1$ to yield a continuous map $f:\C\to S^1$. \textit{later}
	\end{proof} 
