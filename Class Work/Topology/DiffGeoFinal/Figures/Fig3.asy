...
pen g = rgb(103/255,244/255,152/255)+linewidth(0.5);
...
for(int i = 0; i < A; ++i){
	for(int j = 0; j < B; ++j){
		...
		triple vec1 = length*(cos(theta) * (R1*cos(phi + pi/2)), sin(theta)*(R1*cos(phi + pi/2)),R1*sin(phi + pi/2));
		triple vec2 = length*(-1 * sin(theta),cos(theta), 0);
		draw(P--(P + vec1), arrow=Arrow3(emissive(r)), r);
		draw(P--(P + vec2), arrow=Arrow3(emissive(g)), g);
	}
}
