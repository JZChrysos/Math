...
\begin{document}
	...
	\textbf{Proposition (Normal Neighborhood fact)}: Let $U_x$ be a normal neighborhood containing point $x$ with corresponding value $\ep_x$, and let $q\in U_x$. Then
	\begin{enumerate}[(i)]
		\item There is a unique geodesic passing through $x,q$ with length less than $\ep_x$.
		\item This geodesic minimizes distance among all smooth curves from $x$ to $q$.
	\end{enumerate}
	\begin{proof}
