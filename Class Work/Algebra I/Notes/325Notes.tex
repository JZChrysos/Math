...
\begin{proof}
	It is clear that 
	$$\exp(U\cap \Lie(G))\subset \exp(U)\cap \exp(\Lie(G)) \subset \exp(U)\cap G.$$
	The other direction is the interesting one. How can we choose $U$ so that if $g=e^u$ then $u$ must be in $\Lie(G)$?\\
	
	Pick a complementary subspace $V\subset M_n$ of $\Lie(G)$, so that 
	$$
	\Lie(G)\oplus V = M_n.
	$$
	Recall the map
	$$
	\Phi:(u,v)\mapsto e^ue^v.
	$$
	Let $U_m$ be the disk of radius $\tfrac{1}{m}$ in $\Lie(G)$, and similarly $V_m$ in $V$. We claim that for all sufficiently large $m$, 
	$$
	G\cap \Phi(U_m\times V_m)\subset \Phi(U_m\times \{0\}).
	$$
	If this is not true, then there is a sequence $m_i\to \infty$ and $g_i=e^{u_i}e^{v_i}\in G$ for some $|u_i|,|v_i|<\tfrac{1}{m_1}$ with $v_i\neq 0$. By compactness, we can take a subsequence where $v_i/|v_i|$ converges to some limit $|v|=1$, since the sphere is compact. Now by the Limit Lemma, $v\in \Lie(G)$. But $V\cap \Lie(G)=\{0\}$ so $v=0$, and this contradicts $|v|=1$. Thus we have this claim.\\
	
	Let $m$ be sufficiently large so that the claim holds. Since $\exp$ is a local diffeomorphism, we can find an open neighborhood of $0$ in $U\subset U_{m}\times V_{m}$ so that $\exp$ is invertible on $U$. For this $U$, we claim that $\exp(U\cap \Lie(G))=\exp(U)\cap G$.
	
	Suppose that $g\in \exp(U)\cap G\subseteq \Phi(U_m\times V_m)\cap G$. Then because $\exp$ is a bijection, there is a \textit{unique} way to write $g$ as $e^u$, so (revisit!)
	
\end{proof}\\

We can also think of the Lie algebra $\Lie(G)$ as the tangent space at 1 to $G$. For example the Lie algebra of $S^1\subset \C$ is the complex numbers $x$ so that $e^{tx}\in S^1$ for all $t\in \R$, i.e. $x\in i\R$. Note that this is the tangent space to $S^1$ at 1. \\

It is an important fact that $\Lie(G)$ is closed under conjugation by elements in $G$.\\

The \textit{Lie Bracket} is $[x,y]:=xy-yx$. The Lie bracket of two elements in $\Lie(G)$ is in $\Lie(G)$.
\begin{proof}
	If $x,y\in \Lie(G)$, then $e^{tx}$ and $e^{ty}$ are both in $G$ for all $t\in \R$. Thus, $e^{tx}ye^{-tx}\in \Lie(G)$ as well. And when we expand this, we get
	$$
	(1+tx+\tfrac{t^2}{2}x^2 + \cdots ) y (1-tx+\tfrac{t^2}{2}x^2 - \cdots ) = y + t(xy-yx) + \cdots
	$$
	and in particular the derivative in $t$ at $t=0$ is $xy-yx$. So $xy-yx$ is in the tangent space, i.e. it is in $\Lie(G)$.
\end{proof}\\

A morphism of Lie groups is a continuous group homomorphism. A morphism of Lie algebras is a linear map $\ph:\Lie(G)\to \Lie(H)$ which preserves the Lie bracket.\\

\textbf{Proposition}: If $f:G\to H$ is a morphism of Lie groups, then there is a unique algebra morphism $\d f:\Lie(G)\to \Lie(H)$ such that $f(e^x)=e^{\d f(x)}$ for all $x\in \Lie(G)$. Moreover, $\Lie(\ker(f))=\ker(\d f)$. 
\begin{proof}
	Fixing $x\in \Lie(G)$, we have the following sequence of maps:
	$$
	(\R,+) \xrightarrow{e^{\bullet x}} G \xrightarrow{f} H \xhookrightarrow{} \GL_n
	$$
	We showed on homework that any continuous group homomorphism $\R\to M_n^{\times}$ must be of the form $t\mapsto e^{t\cdot v}$ for some $v\in M_n^{\times}$. Call this $v$ $\d f(x)$. Thus we have $f(e^{tx})=e^{t\cdot \d f(x)}$. This satisfies the desired property that $f(e^x)=e^{\d f(x)}$ (take $t=1$). If $f$ were differentiable then we could say
	$$
	\d f(x) = \frac{\d}{\d t}\Big|_{t=0}f(e^{tx})
	$$
	but we do not know this.\\
	
	Now we will show that this constructed map $\d f$ is actually a Lie algebra morphism. First, it is linear (I didn't get what he did to show this, maybe revisit later).\\
	
	Next, we show that $\d f$ preserves the Lie bracket. Let $g\in G$ and $y\in \Lie(G)$. Then
	$$
	e^{t\d f(gyg^{-1})}=f(e^{t(gyg^{-1})})= f(ge^{ty}g^{-1}) = f(g)f(e^{ty})f(g)^{-1} = f(g)e^{t \d f (y)}f(g)^{-1} = e^{t(f(g)\d f(y) f(g)^{-1})}
	$$
	for all $t\in \R$, which implies
	$$
	\d f(gyg^{-1}) = f(g)\d f(y) f(g)^{-1}.
	$$
	Let $g=e^{tx}$. Expanding the LHS gives $\d f([x,y])$ and expanding the RHS gives $[\d f (x),\d f(y)]$.
\end{proof}\\

\subsection{Representations of Lie Groups and Lie Algebras} Let $V$ be a finite-dimensional complex vector space $V\cong \C^n\cong \R^{2n}$ and consider $\GL(V)\subset \GL_{2n}(\R)$ as a Lie group.  \\

Let $G$ be a Lie group in $\GL(V)$. A \textit{continuous} representation of $G$ in $V$ is a morphism of Lie groups $\rho: G\to \GL(V)$ (so it must be continuous). For each representation $\rho$ there is a corresponding $\d \rho:\Lie(G)\to \End(V)$.\\

\textbf{Proposition}: Let $\rho:G\to \GL(V)$ be a continuous representation. Then
\begin{enumerate}[(i)]
	\item If $W\subseteq V$ is $G$-stable then $W$ is $\Lie(G)$-stable as well.
	\item If $f:V\to V$ is a $G$-intertwiner, then $f$ is a $\Lie(G)$-intertwiner as well.
\end{enumerate}
Moreover, if $G$ is connected, then the converses of (i) and (ii) hold.
\begin{proof}
	(i): Suppose $W$ is $G$-stable. We want to show that if $x\in \Lie(G)$ then $\d \rho(x)(w)\in W$ for $w\in W$. $x\in \Lie(G)$ implies $e^{tx}\in G$, so we know that 
	$$
	e^{t\d \rho(x)}(w) = \rho(e^{tx})(w) \in W
	$$
	and taking the derivative of this at $t=0$, we get $\d\rho(x)(w)\in W$.\\
	
	(ii): Let $f:V\to V'$ be a $G$-intertwiner. Then for $x\in \Lie(G)$ we have
	$$
	f \circ e^{t\d \rho(x)} = f\circ \rho(e^{tx}) = \rho(f(e^{tx}))= e^{t\d\rho'(x)} \circ f.
	$$
	so again taking the derivative in $t$ we see that $f \circ \d\rho(w) = \d \rho'(x)\circ f$.\\
	
	To show the converses, we use the following fact: if $G$ is connected then every element of $G$ can be written as a product $g=e^{x_1}e^{x_2}\cdots e^{x_n}$ for some $x_1,\dots,x_n\in \Lie(G)$. On homework we showed that for connected $G$, every element can be expressed as a product of elements in $U$, if $U$ is any open neighborhood of 1. And we know that there is some $\exp(U)$ that is an open neighborhood of $1$ in $G$ by Cartan's Theorem, so this claim follows.
	
	Now, suppose $W\subseteq V$ is $\Lie(G)$-stable. Then for any $x\in \Lie(G)$, $x$ preserves $W$, but this also implies that $e^{x}$ preserves $W$ (since it is a sum of powers of $x$). Now, every $g\in G$ can be written as a product of exponentials $e^{x}$, each of which preserves $W$, so $g$ also preserves $W$.
	
	For the converse of (ii), suppose $\rho,\rho'$ are representations and $f:V\to V'$ is a $\Lie(G)$-intertwining operator. For any $g\in G$, we will show that $f(\rho(g)\cdot v))= \rho(g)\cdot f(v)$. Again, this follows because $g$ can be written as a product of $e^x$, which we know can be commuted with $f$. Commute each of them one-by-one and the result follows.
\end{proof}\\

The significance of this proposition is that we can now study the representation theory of connected Lie groups through their (much simpler) Lie algebras. The irreps of $G$ correspond to irreps of $\Lie(G)$, and if $G$ is \textit{simply connected} then the correspondence is bijective\\

\textbf{Puzzle}: What is the complexification of $S_n$? (informal question)\\

\subsection{Complex Lie Groups} A $\C$-vector space is the same thing as an $\R$-vector space $M$ along with an $\R$-linear operator $I:M\to M$ such that $I^2=-\id$ (which corresponds to multiplication by $i$). In this way, we can say that a map $a:V\to V$ is $\C$-linear if it commutes with addition, scaling by $\R$, \textit{and} the operator $I$.\\

If $V$ is a $\C$-vector space, we define $V_{\R}$ to be the $\R$-vector space given by forgetting the operator $I$. We say that 
$$
\GL(V) := \{g \in \GL(V_\R) \; |\; gI = Ig\}.
$$
We can compute the Lie algebra of this $\GL(V)$ as 
$$
\Lie(\GL(V)) = \{x\in \End_{\R}(V_{\R})\; |\; e^{tx}\in \GL(V) \; \forall t\in \R\} = \Lie(\GL(V_{\R})) \cap \{e^{tx}Ie^{-tx} = 1\} = \End_{\C}(V).
$$
%
%Let $e_1,\dots,e_n$ be a basis of $V=\C^n$, with $e_1,ie_1,\cdots,e_n,ie_n$ the corresponding basis of $V_{\R}$. If $f:V_{\R}\to V_{\R}$ is an $\R$-linear map, then $f$ is $\C$-linear iff \\

If $V$ is a $\C$ vector space, a closed subgroup $G\subset \GL(V)$ is called a \textit{complex Lie group} if $\Lie(G)$ is a complex vector space. For example if $G=S^1\subset \GL_1(\C)$ then $\Lie(G)=i\R$, but this is \textit{not} a complex vector space so $G$ is not a complex Lie group.\\

A representation $\rho:G\to \GL(V)$ of a complex Lie group $G$ is called \textit{holomorphic} if $\d \rho:\Lie(G)\to \End(V)$ is $\C$-linear. For example, the representation $\det: \GL_n(\C)\to \C^*$ has $\d(\det)=\tr$, which is $\C$-linear hence $\det$ is a holomorphic representation. But $\overline{\det}$ and $|\det|$ are not holomorphic.\\

A complex Lie group $G$ is \textit{reductive} if it has a compact subgroup $K\subset G$ such that $\Lie(G)=\Lie(K)+i\Lie(K)$. For example, $G=\OO_n(\C)$ has $\Lie(G)$ as the complex skew-symmetric matrices. These can be decomposed into real and imaginary skew-symmetric parts, so $G$ is reductive.\\

\textbf{Unitary Trick}: Any holomorphic representation of a reductive, connected group $G$ is completely reducible.
\begin{proof}
	Let $\rho:G\to \GL(V)$ be a holomorphic representation. Let $W\subseteq V$ be a $G$-stable subspace. Because $G$ is reductive it has such a compact subgroup $K$. Representations of $K$ are completely reducible because $K$ is compact. Thus if $W'$ is a complement of $W$ in $V$, then $V=W\oplus W'$, so $W'$ is $K$-stable as well.\\
	
	Now, $\d\rho(x)$ preserves $W'$ for $x\in \Lie(K)$. For general $a=x+iy$ where $x,y\in \Lie(K)$, 
	$$
	\d\rho(a)(W') = \d\rho(x)(W') + i\d\rho(y)(W') \subset W'
	$$
	(note that we use the fact that $\rho$ is holomorphic to take $i$ out of $\d\rho(iy)$) so $W'$ is $\Lie(G)$-stable as well. Since $G$ is connected it follows that $W'$ is $G$-stable, thus $\rho$ is completely reducible.
\end{proof}\\

Consider the $\C$-bilinear form $M_n(\C)\times M_n(\C)\to \C$ given by $(a,b)\mapsto \tr(ab)$. If $a,b$ are skew-hermitian (i.e. in $\Lie(U_n)$), then
\begin{align*}
\tr(ab) &= \sum_i a_{ii}b_{ii} + \sum_{i<j} a_{ij}b_{ji} + \sum_{i>j} a_{ij}b_{ij} \\
&= \sum_i a_{ii}b_{ii} + 2 \Re\bigg(\sum_{i<j} a_{ij}b_{ji}\bigg)
\end{align*}
Noting that $a_{ii},b_{ii}\in i\R$, we see that $\tr(ab)\in \R$ in this case. Moreover $\tr(a^2)\leq 0$. Thus $-\tr(a,b)$ is a positive-definite inner product.\\

\textbf{Proposition}: $\GL_n(\C)$ is reductive.
\begin{proof}
	In the case $n=1$, $\GL_n(\C)=\C^{\times}$. $S^1$ is a compact subgroup with Lie algebra $i\R$, so 
	$$
	\Lie(\GL_n(\C)) = \C= i\R \oplus \R = \Lie(S^1) + i\Lie(S^1)
	$$
	thus $\GL_n(\C)$ is reductive.\\
	
	For general $n$, $\Lie(\GL_n(\C))=M_n(\C)$. We want to split this into a real and imaginary part. If we take $U_n$, this is compact and as we showed on homework $\Lie(U_n)$ is the set of skew-hermitian matrices. So
	$$
	\Lie(\GL_n(\C)) = M_n(\C) = \Lie(U_n) \oplus i\Lie(U_n) 
	$$
	because for any $a\in M_n(\C)$,
	$$
	a = \bigg(\frac{a-a^*}{2}\bigg) + i\bigg(-i\frac{a+a^*}{2}\bigg).
	$$
\end{proof}\\

Now back to polynomials. Let $P_d\subset \C[x_1,\dots,x_n]$ be the space of degree $d$ homogeneous polynomials over $\C$. $\GL_n(\C)$ acts on $P_d$, thus giving a representation $\rho_d$ of $\GL_n(\C)$. We will show that this representation is holomorphic.
\begin{proof}
	We need to check that $\d \rho_d: \M_n(\C)\to \End_{\C}(P_d)$ is $\C$-linear.\\
	
	First consider $n=2$. $P_d$ has basis $x^d,x^{d-1}y,\dots,y^d$, so let $f(x,y)=x^iy^j$. For $u\in M_2(\C)$ with entries
	$$
	u = \begin{bmatrix}
		a & b \\
		c & d
	...
	we can calculate
	\begin{align*}
	(\d \rho (u))(f) &= \frac{\d}{\d t} \rho(e^{tu})f\big|_{t=0} \\
	&= \frac{\d}{\d t} f\bigg(e^{-tu}\begin{pmatrix}
		x\\ y
	\end{pmatrix}\bigg)\big|_{t=0}\\
	&= \frac{\d}{\d t} f\bigg(\begin{pmatrix}
		x\\ y
	\end{pmatrix} - t\begin{pmatrix}
	ax+by\\ cx+dy
	\end{pmatrix} + o(t)\bigg)\big|_{t=0}\\
	&= \frac{\d}{\d t} (x-t(ax+by)+o(t))^i(y-t(cx_dy)+o(t))^j\big|_{t=0}\\
	&= (x-t(ax+by)+o(t))^i(y-t(cx+dy)+o(t))^j\big|_{t=0}\\
	&= 
	\end{align*}
	and we can see that this is linear in $a,b,c,d$ and thus linear in $u$ as desired.
\end{proof}\\

As a corollary, for any reductive subgroup of $\GL_n(\C)$, the representation by $P_d$ is holomorphic and hence completely reducible by the unitary trick.\\

\subsection{Holomorphic Irreps of $\GL_n(\C)$} Because we have the unitary trick, we know that holomorphic representations of $\GL_n(\C)$ are completely reducible. But what are these representations?\\

Let $T$ be the torus $(\C^*)^n$.\\

\textbf{Lemma}: Any holomorphic homomorphism $T\to \C^*$ has the form $\chi:t\mapsto t^{\lambda}$ for $\lambda\in \Z^n$.
\begin{proof}
	Holomorphic means that $\d \chi: \C^n\to \C$ is linear, thus $\d\chi$ is an inner product with some $(\lambda_1,\dots,\lambda_n)$. $\chi$ is thus an exponential of this, but it doesn't work unless $\lambda_j$ are integers (details omitted).
\end{proof}\\

Let $\rho:\GL_n(\C)\to \GL(M)$ be a holomorphic representation. Let $T$ be the matrices in $\GL_n$ with diagonal $t_1,\dots,t_n$. We can restrict $\rho$ to $T$ to give a representation of $T$. We decompose $M$ into pieces
$$
M = \bigoplus_{\lambda\in \Z^n} M_{\lambda}
$$
where 
$$
M_{\lambda} = \{m\in M : \rho(t)m = t^{\lambda m}\}.
$$
We say $\lambda$ is a ``weight'' of $M$ if $M_{\lambda}\neq 0$. This decomposition is called the \textit{weight space} of $M$.\\

Let $N$ be the set of upper triangular matrices with 1 on the diagonal. We say that $\lambda$ is a ``highest weight'' of $M$ if $\rho(g)$ fixes some $m\in M_{\lambda}$ for all $g\in N$. We say that $\lambda$ is ``dominant'' if it is ordered, i.e. $\lambda_1\geq \cdots \geq \lambda_n$.\\

\textbf{Classification Theorem}: For every dominant $\lambda$, there is a unique irreducible representation $M(\lambda)$ with highest weight $\lambda$. Moreover, $\lambda$ is the only highest weight of $M(\lambda)$, and $\dim(M(\lambda))=1$.\\

Let $h$ be the diagonal matrix with diagonal $h_1,\dots,h_n$. Let $E_i=1_{i,i+1}$ and $F_{i}=1_{i+1,i}$. Then
$$
[h,E_i] = (h_i-h_{i+1})E_i, \;\; [h,F_i] =-(h_i-h_{i+1})F_i
$$
Let $\a_i$ be the function on diagonal matrices which sends $h$ to $h_i-h_{i+1}$.

Let $\phi:\gl_n(\C)\to \End(V)$ be a finite-dimensional representation of $\gl_n(\C)=M_n(\C)$. Denote $\hat{x}=\phi(x)$.
$$
[\hat{h},\hat{E_i}] = \a_i(h)\hat{E_i} \implies \hat{h}\hat{E_i} = \hat{E_i}(\hat{h} + \a_i(h) I).
$$
Let $\mu$ be a linear function on diagonal matrices, $\mu(h) = \sum \mu_ih_i$. The $\mu$-\textit{weight space} of $V$ is 
$$
V_{\mu} := \{v\in V \; | \; \hat{h} v = \mu(h) v \;\; \forall h \; \text{diagonal}\}
$$
So if $v\in V_{\mu}$ then 
$$
\hat{h}\hat{E_i}v = \hat{E_i}(\hat{h} + \a_i(h)I)v =(\mu+\a_i)(h) \hat{E_i}v.
$$
That is, $\hat{E_i}(V_{\mu})\subseteq V_{\mu + \a_i}$, and similarly for $\hat{F_i}$. 

Let $g_+$ be the upper triangular matrices with diagonal 0, and similarly $g_-$. We say that a nonzero vector $v\in V_{\mu}$ is a \textit{highest weight vector} of weight $\mu$ if $\hat{X}v=0$ for all $X\in g_+$. Or equivalently, if 
$$\widehat{1_{i,j}}v=\begin{cases}
	0 & i<j\\
	\mu_{i}v & i=j
\end{cases}$$
Let $v\in V_{\lambda}$ be a highest weight vector of weight $\lambda$. Let $M$ be the $\C$-span of $v,\hat{F_i}v,\hat{F_i}\hat{F_j}v,\dots$. We just showed that applying $\hat{F_i}$ reduces the weight by $\a_i$ for any $i$. So
$$
M = \bigoplus_{\mu} M_{\a-\mu}
$$
where $\mu$ ranges over $m_1\a_1+m_2\a_2+\cdots+m_n\a_n$ for $m_i\in \N$.\\

\textbf{Proposition}: $M$ is $\gl_n$-stable.
\begin{proof}
	We just need to show that $\widehat{1_{i,j}}(M)\subseteq M$ for $i,j$.\\
	
	In the case $i=j$, $\widehat{1_{i,i}}(M_{\lambda - \mu}) = (\lambda_i - \mu_i)M_{\lambda - \mu} \subset M_{\lambda - \mu}$.\\
	
	In the case $i>j$, we will show by induction on $i-j$. We showed the case $i-j=0$ already. If $i=j+1$, then $1_{i,j} = F_i$ by definition of $F_i$, so clearly $\hat{F_i}$ only reduces weights.\\
	
	For the inductive step, note that $1_{ij}=[F_{i-1},1_{i-1,j}]$. Because both $\widehat{F_{i-1}}$ and $\widehat{1_{i-1,j}}$ preserve $M$ (by inductive hypothesis), it follows that their commutator does, so $\widehat{1_{ij}}$ does.\\
	
	In the case $i<j$, we again use induction. In the case $j=i+1$, then $1_{i,j}=E_i$, so $\hat{E_i}$ adds weight, thus this is less immediate than the $F_i$ case. We have for some diagonal $h$,
	$$
	\hat{E_i}\hat{F_j}v = \hat{F_j}\hat{E_i}v + \hat{h}v = 0 + (\lambda - \mu)(h)v \in \C \cdot v.
	$$
	using the fact that $v$ is a highest-weight vector, so it gets killed by $\hat{E_i}$. The inductive step is similar.
\end{proof}\\

\textbf{Proposition}: Let $\rho:\GL_n(\C)\to \GL(V)$ be a holomorphic representation and $v\in V_{\lambda}$ a highest weight vector for $\d\rho: \gl_n \to \End(V)$. Let $M$ again be the $\C$-vectorspace generated by $v$ and $\hat{F_i}$ for all $i$. Then $M$ is a $\GL_n$-stable subspace of $V$. Moreover, $M$ is an irrep of $\GL_n(\C)$.
\begin{proof}
	By the previous proposition $M$ is $\gl_n$-stable, so it follows that $M$ is $\GL_n$-stable. Similarly if $M$ is a $\gl_n$-irrep then it is a $\GL_n$-irrep. So we just need to show that $M$ is a $\gl_n$-irrep.\\
	
	Assume that $M'\subseteq M$ is $\GL_n$-stable. Since $\GL_n(\C)$ is reductive, there is a $\GL_n$-stable complement of $M'$ in $M$, which we can call $M''$, so that $M=M'\oplus M''$. Then we also have
	$$
	M_{\lambda} = M'_{\lambda} \oplus M''_{\lambda}
	$$
	for each $\lambda$. But now we claim that $M_{\lambda}=\C v$ is one-dimensional (why?).\\
	
	Now for any $\lambda$, either $M'_{\lambda}$ or $M''_{\lambda}$ is trivial. Then we can show that $M''=0$ (details fill in later)
\end{proof}\\

\subsection{Tensors of representations}

Let $G$ be a Lie group and $\rho,V$, $\rho',V'$ two representations of $G$. Then we can define the tensor product of these two representations as
$$
(\rho\otimes \rho')g: v\otimes v' \mapsto \rho(g)v\otimes \rho'(g)v'.
$$
What is $\d(\rho\otimes \rho')$? It turns out to be
$$
\d(\rho\otimes \rho') = \d\rho \otimes \id_{V'} + \d\rho' \otimes \id_{V}.
$$
To show this, we can calculate out how $e^{t\d(\rho\otimes \rho')(x)}$ acts on $v\otimes v'$ and take the derivative in $t$.\\

As a corollary, if $v,v'$ are eigenvectors of $\d\rho$ and $\d\rho'$ with eigenvalues $\lambda$ and $\mu$, then $v\otimes v'$ is an eigenvector of $\d(\rho \otimes \rho')$ with eigenvalue $\lambda + \mu$. This will be an important fact.\\

Returning to the setting of $\GL_n(\C)$, let $G=\GL_n(\C)$ with two holomorphic representations $\rho,V$ and $\rho',V'$. As before, for each $\lambda = \lambda_1,\dots,\lambda_n$, let
$$V_{\lambda} := \{v\in V \; | \; \d \rho (1_{ii})v = \lambda_i v \; \forall i\}$$
and similarly for $V'_{\mu}$. Using the tensor fact we just proved, we have
$$
V_{\lambda}\otimes V'_{\mu} \subseteq (V\otimes V')_{\lambda+\mu}.
$$
If $v\in V_{\lambda}$ and $v'\in V'_{\mu}$ are both highest weight vectors, then $v\otimes v'$ is highest weight in $(V\otimes V')_{\lambda + \mu}$.\\

Examples:
\begin{itemize}
	\item If $V=\C^n$ then the highest-weight vector is $e_1$, and its weight is $(1,0,\dots,0)$.
	\item If $V=\wedge^2\C^n$, then the highest-weight vector is $e_1\wedge e_2$, with weight $(1,1,0,0,\dots,0)$.
	\item If $V=\wedge^p\C^n$, then the highest-weight vector is $e_1\wedge\cdots\wedge e_p$, with weight $(1,1,1,\dots,1,0,0,\dots,0)$.\\
\end{itemize}

In general, let $\omega_j$ be a tuple of $j$ 1s followed by 0s. These are the ``fundamental weights'' and are important.\\ 

\textbf{Theorem}: For all $\lambda = (\lambda_1\geq \lambda_2\geq \cdots \lambda_n)$ there is a \textit{unique} holomorphic irrep of $\GL_n$ with highest weight $\lambda$.
\begin{proof}
	$\lambda$ can be written as 
	$$
	\lambda_n\omega_n + (\lambda_{n-1}-\lambda_n)\omega_{n-1} + \cdots + (\lambda_1-\lambda_2) \omega_1.
	$$
	We've seen that we can achieve $\omega_1$ as a highest-weight of $V$ and $\omega_2$ as a highest-weight of $\wedge^2 V$ and so on. So we can make a representation 
	$$
	M(V,v) := \Big(\wedge^1 V\Big)^{\otimes (\lambda_1 - \lambda_2)} \otimes \Big(\wedge^2 V\Big)^{\otimes (\lambda_2 - \lambda_3)} \otimes \cdots \otimes \Big(\wedge^n V\Big)^{\otimes (\lambda_n)}
	$$
	with highest-weight $\lambda$. Note that for $\lambda_n$ it is okay that it may be negative (why?). \\
	
	Now we show that this representation is unique. Let $W$ be an irrep with highest weight vector $w\in W_{\lambda}$. Take $W\oplus M(V,v)$. The highest weight vector in this direct sum is $(w,v)$. Then we can also take
	$$
	M(W\oplus M(V,v),(w,v))
	$$
	which is irreducible. I didn't f
\end{proof}
\end{document}